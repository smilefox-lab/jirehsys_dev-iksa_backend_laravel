...
class DashboardController extends BaseController
{
    ...
    public function getDashboard(Request $request)
    {
        ...
        $properties = Property::query()->select('id', 'name', 'coordinates', 'location', 'description');

        if (!Auth::user()->inRole('admin') && !Auth::user()->isSuperUser()) {
            $properties = $properties->where('company_id', '=', Auth::user()->company_id);
        }

        $properties = $properties->get();

        return view('core/dashboard::list', compact('widgets', 'userWidgets', 'properties'));
    }
