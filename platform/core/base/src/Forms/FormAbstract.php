...
abstract class FormAbstract extends Form
{
     ...
     * @return void
     */
    public function addOptionsInFields(array $names, array $options = [])
    {
        foreach (is_array($names) ? $names : func_get_args() as $name) {
            if ($this->has($name)) {
                $optionsOrigin = $this->fields[$name]->getOptions();
                foreach ($options as $key => $option) {
                    if (Arr::exists($optionsOrigin, $key)) {
                        if (is_array($optionsOrigin[$key])) {
                            $optionsOrigin[$key] += $option;
                        } else {
                            $optionsOrigin[$key] = $option;
                        }
                    }
                }
                $this->fields[$name]->setOptions($optionsOrigin);
            }
        }
