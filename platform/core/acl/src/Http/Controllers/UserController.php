...
class UserController extends BaseController
{
    ...
    public function getUserProfile($id, Request $request, FormBuilder $formBuilder)
    {
        ...
        if (!$currentUser->isSuperUser()) {
            $form->addOptionsInFields(['username', 'email'], [ 'attr' => [ 'readonly' => true ]]);
            $form->addOptionsInFields(['company_id'], [ 'attr' => [ 'disabled' => true ]]);
        }
    ...
    public function postUpdateProfile($id, UpdateProfileRequest $request, BaseHttpResponse $response)
    {
        ...
        if ($request->user()->getKey() != $id && !$request->user()->isSuperUser()) {
            return $response
                ->setError()
                ->setCode(403)
                ->setMessage(__('No tiene permiso para editar estos datos.'));
        }
        ...
        if (($currentUser->hasPermission('users.update-profile') && $currentUser->getKey() === $user->id) ||
            ...
            // validar que el usuario no pueda cambiar ni el username, ni el
            // email
        }
